...
士兵的双腿平衡不是靠真实的动力学平衡（如倒立摆控制），而是由以下三个机制组合实现的：

### 13.1 旋转弹簧（核心）

在 `AHuman::Update()` 中，每帧对躯体施加一个角度恢复力：

```cpp
// Source/Entities/AHuman.cpp，Update() 中约第 2555 行
if (m_Status == STABLE) {
    float rotTarget = GetRotAngleTarget(m_MovementState) * ...;  // 目标直立角（约为 0）
    float rotDiff = rot - rotTarget;
    // 阻尼弹簧：角速度以 0.98 衰减，同时向目标方向施加修正力
    m_AngularVel = m_AngularVel * (0.98F - 0.06F * (m_Health / m_MaxHealth))
                 - (rotDiff * 0.5F);
}
```

这个弹簧每帧把身体"拉"回直立，系数 0.5 控制弹簧刚度，0.98 提供阻尼防止过冲。

### 13.2 脚踩地面产生支持力

站立时（`STAND` 状态），两条腿的 `AtomGroup` 每帧调用 `PushAsLimb()`，脚被推到髋关节正下方的固定点。脚的原子碰到地形后停下，地面给脚的支持力（法向反作用力）通过 `AddImpulseForce()` 传递给躯体，抵消重力。

```
重力 ↓         ↑ 地面支持力（通过 AddImpulseForce 传给躯体）
躯体 ──── 前景脚 ──── 地形
     ──── 背景脚 ──── 地形
```
...
- **STABLE**：旋转弹簧拉向直立（上面 13.1 的逻辑），士兵正常站立
- **UNSTABLE**：弹簧改为拉向倒地方向，肢体切换为布娃娃（`FlailAsLimb`）
- 判据：X 或 Y 方向速度超过 `m_StableVel`（默认 15/25 m/s）则进入 UNSTABLE

### 平衡机制一句话总结

> 旋转弹簧维持直立姿态 + 两脚的地面支持力抵消重力 = 双腿站立平衡。

---

> **Godot 实现指南** 请参阅 [`Documentation/GodotSoldierImplementation_CN.md`](GodotSoldierImplementation_CN.md)，其中包含完整的 GDScript 代码示例，涵盖旋转弹簧、腿部 IK、路径点脚部控制器和坡度检测。
